...
function mapMediaContent(content: any, document: any) {
  ...
  return Object.entries(content).map(([mediaType, mediaEntry]: [string, any]) => {
    ...
    const examples = [...collectExamples(resolved?.examples), ...collectExamples(schema?.examples)];
    const example =
...
function buildSnippets({ method, url, headers, bodyExample }: any) {
  ...
  const csharpMethod =
    methodUpper === "PATCH" ? "Patch" : methodUpper.charAt(0) + methodUpper.slice(1).toLowerCase();
  const csharpSnippet = [
...
function mapOperation({
  ...
  const mappedParameters = mergedParameters.map((parameter: any) => {
    ...
    const examples = [...collectExamples(parameter.examples), ...collectExamples(schema?.examples)];
    const example =
