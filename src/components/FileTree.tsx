...
export function FileTree({ children }: { children: React.ReactNode }) {
  return (
    <div className="not-prose rounded-lg border border-border bg-card px-2 py-4 font-mono text-sm text-card-foreground shadow-sm">
      <ul className="m-0 flex flex-col p-0">{children}</ul>
    </div>
  );
}
...
export function Folder({
  ...
  return (
    <li className="m-0 flex flex-col p-0">
      ...
      {isOpen && <ul className="m-0 ml-3 flex flex-col border-l border-border pl-3">{children}</ul>}
    </li>
  );
}
...
export function File({ name }: { name: string }) {
  return (
    <li className="m-0 flex items-center gap-2 rounded-md p-0 px-2 py-1 text-muted-foreground transition-colors select-none hover:bg-muted hover:text-foreground">
