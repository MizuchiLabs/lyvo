...
export async function getDocsHierarchy() {
   ...
   const topLevelItems = [
      ...
      ...Object.keys(categorizedDocs)
         ...
         .map((catId) => {
            const label = labelsObj[catId] || catId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            return {
               id: label,
...
export async function getPrevNextDocs(currentId: string) {
   ...
   const nextDoc = currentIndex < sortedDocs.length - 1 && currentIndex !== -1 ? sortedDocs[currentIndex + 1] : null;
